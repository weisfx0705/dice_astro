...
「你問我該不該接受這份工作？從骰子來看，答案是可以。但摩羯座和第十宮的組合意味著你必須準備好承受壓力，並且你將無法輕易逃避責任。這份工作會給你成長的機會，但同時也會榨乾你的精力。所以，如果你準備好在工作中磨練自己，並且承受上司的高壓，那麼就勇敢地接受吧。只是不會有太多快樂時光。別指望在這裡找到什麼輕鬆愉快的日子。」`
    ...
    function initializeCardModal() {
        ...
        document.addEventListener('click', function(event) {
            if (event.target.classList.contains('card-image-small') && event.target.classList.contains('clickable')) {
                ...
                switch(cardType) {
                    case 'planet':
                        if (selectedCards.planet) {
                            cardInfo = cardData.planet.find(p => p.id === selectedCards.planet);
                            cardName = cardInfo.name;
                        }
                        break;
                    case 'star':
                        if (selectedCards.star) {
                            cardInfo = cardData.star.find(s => s.id === selectedCards.star);
                            cardName = cardInfo.name;
                        }
                        break;
                    case 'house':
                        if (selectedCards.house) {
                            cardInfo = cardData.house.find(h => h.id === selectedCards.house);
                            cardName = cardInfo.name;
                        }
                        break;
                }
                ...
                if (cardInfo) {
                    ...
                    const modalName = document.querySelector('.card-modal-name');
                    
                    modalImage.style.backgroundImage = `url(${cardType}/${cardType}_${String(cardInfo.id).padStart(2, '0')}.png)`;
